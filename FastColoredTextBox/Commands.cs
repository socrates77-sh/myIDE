            ...
            switch (c)
            {
                ...
                case '\t':
                   ...
                   // break;
					...
					ts[tb.Selection.Start.iLine].Insert(tb.Selection.Start.iChar, new Char('\t', false));
                    for (int i = 0; i < spaceCountNextTabStop-1; i++)
                        ts[tb.Selection.Start.iLine].Insert(tb.Selection.Start.iChar, new Char(' ',true));

                    tb.Selection.Start = new Place(tb.Selection.Start.iChar + spaceCountNextTabStop, tb.Selection.Start.iLine);
                    break;
                default:
                    ts[tb.Selection.Start.iLine].Insert(tb.Selection.Start.iChar, new Char(c,false));
                    tb.Selection.Start = new Place(tb.Selection.Start.iChar + 1, tb.Selection.Start.iLine);
                    break;
            }
        }
