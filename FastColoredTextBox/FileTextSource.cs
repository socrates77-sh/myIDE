...
namespace FastColoredTextBoxNS
{
    ...
    public class FileTextSource : TextSource, IDisposable
    {
        ...
        public FileTextSource(FastColoredTextBox currentTB,int tablength)
            : base(currentTB)
        {
        ...
        public void OpenFile(string fileName, Encoding enc,int tabLength)
        {
            Clear();
            ...
            for (int i = 0; i < linesCount; i++)
                LoadLineFromSourceFile(i,tabLength);
            //
        ...
        private void LoadLineFromSourceFile(int i,int tabLength)
        {
            var line = CreateLine();
			...
			foreach (var c in s)
			{
				if (c.ToString() == "\t")
				{
					int spaces = tabLength-(charindex % tabLength);

					for (int j = 0; j < spaces - 1; j++)
					{
						line.Add(new Char(' ', true));
						charindex++;
					}

					line.Add(new Char('\t', false));
				}
				else
					line.Add(new Char(c, false));
