...
namespace FastColoredTextBoxNS
{
       ...
       // public readonly Style BoldStyle = new TextStyle(null, null, FontStyle.Bold | FontStyle.Underline);
        ...
        public virtual void HighlightSyntax(Language language, Range range)
        {
            switch (language)
            {
                ...
                case Language.ASM: ASMSyntaxHighlight(range); break;
                case Language.RISC: RISCSyntaxHighlight(range); break;
                default:
                    break;
            }
        }
        ...
        public virtual void AutoIndentNeeded(object sender, AutoIndentEventArgs args)
        {
            ...
            switch (language)
            {
                ...
                case Language.ASM: CSharpAutoIndentNeeded(sender, args); break;//
                case Language.RISC: CSharpAutoIndentNeeded(sender, args); break;//
                default:
                    break;
            }
        }
    ...
    brackets.Attributes["left2"].Value == "" || brackets.Attributes["right2"].Value == "")
        ...
        void InitCShaprRegex()
        {
            ...
            CSharpKeywordRegex = new Regex(@"\b(abstract|as|base|bool|break|byte|case|catch|char|checked|class|const|continue|define|default|delegate|do|double|else|enum|event|explicit|extern|false|finally|fixed|float|for|foreach|goto|if|implicit|in|int|include|internal|is|lock|long|namespace|new|null|object|operator|out|override|params|private|protected|public|readonly|ref|return|sbyte|sealed|short|sizeof|stackalloc|static|string|struct|switch|this|throw|true|try|typeof|uint|ulong|unchecked|unsafe|ushort|using|virtual|void|volatile|while|add|alias|ascending|descending|dynamic|from|get|global|group|into|join|let|orderby|partial|remove|select|set|value|var|where|yield)\b|#asm\b|#endasm\b", RegexCompiledOption);
        }
        /// <summary>
        /// Highlights C# code
        /// </summary>
        ...
        public virtual void CSharpSyntaxHighlight(Range range)
        {
           ...
           // range.SetFoldingMarkers(@"#region\b", @"#endregion\b");//allow to collapse #region blocks
            range.SetFoldingMarkers(@"#asm\b", @"#endasm\b");//allow to collapse #region blocks
        ...
        //ASM 2011.11.17-----------------------------------------------------------------------------
        Regex ASMStringRegex, ASMCommentRegex1, ASMCommentRegex2, ASMCommentRegex3, ASMNumberRegex, ASMAttributeRegex, ASMClassNameRegex, ASMKeywordRegex;

        void InitASMRegex()
        {
            ASMStringRegex = new Regex(@"""""|@""""|''|@"".*?""|[^@](?<range>"".*?[^\\]"")|'.*?[^\\]'", RegexCompiledOption);
            ASMCommentRegex1 = new Regex(@";.*$", RegexOptions.Multiline | RegexCompiledOption);
            //ASMCommentRegex2 = new Regex(@"(/\*.*?\*/)|(/\*.*)", RegexOptions.Singleline | RegexOptions.Compiled);
            //ASMCommentRegex3 = new Regex(@"(/\*.*?\*/)|(.*\*/)", RegexOptions.Singleline | RegexOptions.RightToLeft | RegexOptions.Compiled);
            ASMNumberRegex = new Regex(@"\b\d+[\.]?\d*([eE]\-?\d+)?[lLdDfF]?\b|\s.\$[a-fA-F\d]+\b|\b0x[a-fA-F\d]+\b", RegexCompiledOption);
            ASMAttributeRegex = new Regex(@"^\s*(?<range>\[.+?\])\s*$", RegexOptions.Multiline | RegexCompiledOption);
            ASMClassNameRegex = new Regex(@"\b(class|struct|enum|interface|jsr|JSR)\s+(?<range>\w+?)\b", RegexCompiledOption);
            ASMKeywordRegex = new Regex(@"\b(
                    ...
                    |ADDAR|ADDRA|ANDAR|ANDRA|CLRR|CLRA|COMAR|COMR|DECAR|DECR|DJZAR|DJZR|INCAR|INCR|
                    |JZAR|JZR|ORAR|ORRA|MOVAR|MOVR|MOVRA|RLAR|RLR|RRAR|RRR|RSUBRA|RSUBAR|SWAPAR|SWAPR|XORAR|
                    |JBCLR|JBSET|BCLR|BSET|ADDAI|ANDAI|ORAI|MOVAI|RETAI|ISUBAI|XORAI|ADCAI|ISBCAI|ASBCAI|
                    |ASBAI|MULAR|MULRA|ADCAR|ADCAR|ADCRA|RSBCAR|RSBCRA|ASUBAR|ASUBRA|ASBCAR|ASBCRA|CLRWDT|
                    |RETIE|RETURN|DAA|DSA|CALL|GOTO|INCA|JZA|DECA|DJZA|RRA|RLA|ASUBAI|DW|INCLUDE|GLOBAL|
                    |CMPI|CMPR|PUSH|POP|XCH|MOVC|XORRA|
                    )\b|#region\b|#endregion\b", RegexOptions.IgnoreCase | RegexCompiledOption);
        }
        /// <summary>
        /// Highlights ASM code
        /// </summary>
        /// <param name="range"></param>
        public virtual void ASMSyntaxHighlight(Range range)
        {
            range.tb.CommentPrefix = "//";
            range.tb.LeftBracket = '(';
            range.tb.RightBracket = ')';
            ...
            range.ClearStyle(BlueStyle, BoldStyle, GrayStyle, MagentaStyle, GreenStyle, BrownStyle);
            //
            if (ASMStringRegex == null)
                InitASMRegex();
            //string highlighting
            range.SetStyle(BrownStyle, ASMStringRegex);
            //comment highlighting
            range.SetStyle(GreenStyle, ASMCommentRegex1);
            //range.SetStyle(GreenStyle, ASMCommentRegex2);
           // range.SetStyle(GreenStyle, ASMCommentRegex3);
            //number highlighting
            range.SetStyle(MagentaStyle, ASMNumberRegex);
            //attribute highlighting
            range.SetStyle(GrayStyle, ASMAttributeRegex);
            //class name highlighting
            range.SetStyle(BoldStyle, ASMClassNameRegex);
            //keyword highlighting
            range.SetStyle(BlueStyle, ASMKeywordRegex);

            //clear folding markers
            range.ClearFoldingMarkers();
            //set folding markers
            range.SetFoldingMarkers("{", "}");//allow to collapse brackets block
            range.SetFoldingMarkers("fn_|FN_", @"\b(rti|RTI|rts|RTS)\b");//allow to collapse brackets block
            range.SetFoldingMarkers(@"#region\b", @"#endregion\b");//allow to collapse #region blocks
            //range.SetFoldingMarkers(@"/\*", @"\*/");//allow to collapse comment block
        }
        //-end ASM ----------------------------------------------------------------------        //RISC 2011.11.17
        Regex RISCStringRegex, RISCCommentRegex1, RISCCommentRegex2, RISCCommentRegex3, RISCNumberRegex, RISCAttributeRegex, RISCClassNameRegex, RISCKeywordRegex;

        void InitRISCRegex()
        {
            RISCStringRegex = new Regex(@"""""|@""""|''|@"".*?""|[^@](?<range>"".*?[^\\]"")|'.*?[^\\]'", RegexCompiledOption);
            RISCCommentRegex1 = new Regex(@";.*$", RegexOptions.Multiline | RegexCompiledOption);
            //RISCCommentRegex2 = new Regex(@"(/\*.*?\*/)|(/\*.*)", RegexOptions.Singleline | RegexOptions.Compiled);
            //RISCCommentRegex3 = new Regex(@"(/\*.*?\*/)|(.*\*/)", RegexOptions.Singleline | RegexOptions.RightToLeft | RegexOptions.Compiled);
            //RISCNumberRegex = new Regex(@"\b\d+[\.]?\d*([eE]\-?\d+)?[lLdDfF]?\b|\s.\$[a-fA-F\d]+\b|\b0x[a-fA-F\d]+\b", RegexOptions.Compiled);
            RISCNumberRegex = new Regex(@"\b\d+[\.]?\d*([eE]\-?\d+)?[lLdDfF]?\b|\s.\$[a-fA-F\d]+\b|\b0x[a-fA-F\d]+\b|[0-1]+b+\b", RegexCompiledOption);
            RISCAttributeRegex = new Regex(@"^\s*(?<range>\[.+?\])\s*$", RegexOptions.Multiline | RegexCompiledOption);
            RISCClassNameRegex = new Regex(@"\b(class|struct|enum|interface|call)\s+(?<range>\w+?)\b", RegexOptions.IgnoreCase | RegexCompiledOption);
            RISCKeywordRegex = new Regex(@"\b(
                    |DEFINE|DS|DB|ELSE|END|ENDC|ENDIF|ENDM|ENDW|EQU|ERROR|ERRORLEVEL|EXITM|EXPAND|
                    |EXTERN|IDATA|IF|IFDEF|IFNDEF|LIST|LOCAL|MACRO|NOEXPAND |NOLIST|ORG|FDB|FCB|
                    |GFMO|PAGESEL|PROCESSOR|RADIX|RES|RMB|SET|SPACE|CODE|
                    
                    |ADDAR|ADDRA|ANDAR|ANDRA|CLRR|CLRA|COMAR|COMR|DECAR|DECR|DJZAR|DJZR|INCAR|INCR|NOP|
                    |JZAR|JZR|ORAR|ORRA|MOVAR|MOVR|MOVRA|RLAR|RLR|RRAR|RRR|RSUBRA|RSUBAR|SWAPAR|SWAPR|XORAR|
                    |JBCLR|JBSET|BCLR|BSET|ADDAI|ANDAI|ORAI|MOVAI|NO|RETAI|ISUBAI|XORAI|ADCAI|ISBCAI|ASBCAI|
                    |ASBAI|MULAR|MULRA|ADCAR|ADCAR|ADCRA|RSBCAR|RSBCRA|ASUBAR|ASUBRA|ASBCAR|ASBCRA|CLRWDT|
                    |RETIE|RETURN|DAA|DSA|CALL|GOTO|INCA|JZA|DECA|DJZA|RRA|RLA|ASUBAI|DW|INCLUDE|GLOBAL|
                    |CMPI|CMPR|PUSH|POP|XCH|MOVC|XORRA|
                    )\b|#region\b|#endregion\b", RegexOptions.IgnoreCase | RegexCompiledOption);
        ...
        /// <summary>
        /// Highlights RISC code
        /// </summary>
        /// <param name="range"></param>
        public virtual void RISCSyntaxHighlight(Range range)
        {
            range.tb.CommentPrefix = "//";
            range.tb.LeftBracket = '(';
            range.tb.RightBracket = ')';
            ...
            range.ClearStyle(BlueStyle, BoldStyle, GrayStyle, MagentaStyle, GreenStyle, BrownStyle);
            //
            if (RISCStringRegex == null)
                InitRISCRegex();
            //string highlighting
            range.SetStyle(BrownStyle, RISCStringRegex);
            //comment highlighting
            range.SetStyle(GreenStyle, RISCCommentRegex1);
            //range.SetStyle(GreenStyle, RISCCommentRegex2);
            // range.SetStyle(GreenStyle, RISCCommentRegex3);
            //number highlighting
            range.SetStyle(MagentaStyle, RISCNumberRegex);
            //attribute highlighting
            range.SetStyle(GrayStyle, RISCAttributeRegex);
            //class name highlighting
            range.SetStyle(BoldStyle, RISCClassNameRegex);
            //keyword highlighting
            range.SetStyle(BlueStyle, RISCKeywordRegex);

            //clear folding markers
            range.ClearFoldingMarkers();
            //set folding markers
            range.SetFoldingMarkers("#ifndef|#IFNDEF", "#endif|#ENDIF");//allow to collapse brackets block
            range.SetFoldingMarkers("{", "}");//allow to collapse brackets block
            range.SetFoldingMarkers(@"\n(fn_|fun_)", @"\b(return|retie)\b");//allow to collapse brackets block
            range.SetFoldingMarkers(@"cblock\b", @"endc\b");//allow to collapse #region blocks
            //range.SetFoldingMarkers(@"/\*", @"\*/");//allow to collapse comment block
        }        //-end RISC -------------------------------------------------------------------------
    ...
    {
        Custom, CSharp, VB, HTML, SQL, PHP, JS,ASM,RISC
    }
}
